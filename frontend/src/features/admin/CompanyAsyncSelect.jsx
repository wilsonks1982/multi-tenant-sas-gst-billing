...
export default function CompanyAsyncSelect({
  value,
  onChange,
  label = "Company",
  isRequired = false,
  ...
  useEffect(() => {
    const fetchSelectedCompany = async () => {
      ...
      try {
        const res = await api.get(`/api/platform/companies/${value}`);
      ...
      debouncePromise(async (inputValue) => {
        ...
        try {
          ...
          for (const option of options) {
            cacheRef.current.set(`company:${option.value}`, option);
          }
  ...
  return (
    <FormControl isInvalid={!!error} isRequired={isRequired}>
      ...
      <AsyncSelect
        ...
        isClearable
        cacheOptions={false}
        placeholder={`Type at least ${MIN_SEARCH_LENGTH} characters to search company`}
        ...
        onChange={(option) => {
          ...
          onChange?.(option?.value ?? "", option?.company ?? null);
        }}
        chakraStyles={{
          menu: (provided) => ({
            ...provided,
